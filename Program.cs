...
namespace TapoToggle
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var configFile = "config.demo.json";
            if (args.Length > 0)
            {
                var input = args[0];
                configFile = input.EndsWith(".json", StringComparison.OrdinalIgnoreCase) ? input : $"{input}.json";
            }

            if (!File.Exists(configFile))
            {
                ConsoleLog($"   Error: Configuration file '{configFile}' not found.", ConsoleColor.Red);
                ConsoleLog("   Usage: TapoToggle.exe [config-name]");
                return;
            }
            ...
            var email = config["TapoConfig:Email"] ?? "";
            var password = config["TapoConfig:Password"] ?? "";
            var deviceLabel = config["TapoConfig:DeviceLabel"] ?? "";

            if (string.IsNullOrEmpty(email) || string.IsNullOrEmpty(password))
            {
                ConsoleLog($"   Error: Required credentials missing in '{configFile}'.", ConsoleColor.Red);
                return;
            }
            ...
            try
            {
                ...
                ConsoleLog("1. Authenticating with Tapo Cloud...");
                ...
                var device = deviceList.DeviceList.FirstOrDefault(d =>
                    d.Alias.Equals(deviceLabel, StringComparison.OrdinalIgnoreCase));

                if (device == null)
                {
                    ConsoleLog($"   Error: Device named '{deviceLabel}' was not found.", ConsoleColor.Yellow);
                    return;
                }

                ConsoleLog($"   Found Device: {device.Alias} (MAC: {device.DeviceMac})", ConsoleColor.Gray);

                // STEP 3: Network Discovery
                ConsoleLog("3. Resolving Local IP...");

                // Pre-scan to wake up the device and populate ARP
                ConsoleLog("   Performing subnet pre-scan to wake up devices...", ConsoleColor.Gray);
                await PreScanSubnetAsync();

                // Try UDP Discovery first
                ConsoleLog("   Attempting UDP Discovery...", ConsoleColor.Gray);
                var resolvedIp = await DiscoverIpByMacAsync(device.DeviceMac);

                // Fallback to ARP
                if (string.IsNullOrEmpty(resolvedIp))
                {
                    ConsoleLog("   UDP Discovery timed out. Falling back to ARP table...", ConsoleColor.Gray);
                    resolvedIp = ResolveIpFromMac(device.DeviceMac);
                }
        ...
        private static string NormaliseMac(string mac) => mac.Replace("-", "").Replace(":", "").ToLower();
        ...
        private static async Task PreScanSubnetAsync()
        {
            ...
            // Using a semi-parallel approach with controlled disposal to avoid resource leaks
        ...
        private static async Task<string?> DiscoverIpByMacAsync(string macAddress)
        {
            ...
            foreach (var address in GetBroadcastAddresses())
            {
                ...
                catch { /* Continue searching on next interface */ }
            }
